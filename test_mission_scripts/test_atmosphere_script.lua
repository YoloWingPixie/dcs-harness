...
local function samplePoint()
    ...
    if u and u.getPoint then
        local ok, point = pcall(function() return u:getPoint() end)
        if ok and type(point) == "table" and point.x and point.y and point.z then return point end
    end
    return {x = 0, y = 0, z = 0}
end
...
local function logAtmosphere()
    ...
    if type(ret[1]) == "table" then
        local tp = ret[1]
        local tK = tp and (((type(tp.temperatureK) == "number") and tp.temperatureK)
            or ((type(tp.temperature) == "number") and tp.temperature)
            or ((type(tp.temp) == "number") and tp.temp)
            or ((type(tp.t) == "number") and tp.t)) or nil
        local tC = tp and (((type(tp.temperatureC) == "number") and tp.temperatureC)
            or (tK and (tK - 273.15))) or nil
        local pPa = tp and (((type(tp.pressurePa) == "number") and tp.pressurePa)
            or ((type(tp.pressure) == "number") and tp.pressure)
            or ((type(tp.p) == "number") and tp.p)
            or ((type(tp.qnh) == "number") and tp.qnh)) or nil
        local inHg = ((type(tp) == "table") and (type(tp.pressureInHg) == "number") and tp.pressureInHg)
            or (pPa and (pPa / 3386.389)) or nil
        local hPa = ((type(tp) == "table") and (type(tp.pressurehPa) == "number") and tp.pressurehPa)
            or (pPa and (pPa / 100.0)) or nil
        info(string.format("Temp=%.2fC (%.2fF) Pressure=%.2finHg (%.1fhPa)",
    ...
    if type(ret[1]) == "number" and type(ret[2]) == "number" then
        ...
        info(string.format("Temp=%.2fC (%.2fF) Pressure=%.2finHg (%.1fhPa)", tC, (tC * 9/5 + 32), inHg, hPa))
        return
    end
...
local function main()
    ...
    ScheduleOnce(function() logAtmosphere() end, nil, 10)
    ScheduleOnce(function() logAtmosphere() end, nil, 30)
    ScheduleOnce(function() logAtmosphere() end, nil, 60)
end
