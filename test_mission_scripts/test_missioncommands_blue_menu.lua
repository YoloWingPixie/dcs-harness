...
local function setupBlueMenu()
    ...
    local item
    item = CreateMenuItem("Ping")
    if item then
        AddCommandForCoalition(coalition.side.BLUE, util, item, onPing)
    end
    item = CreateMenuItem("Show Wind")
    if item then
        AddCommandForCoalition(coalition.side.BLUE, util, item, onShowWind)
    end
    item = CreateMenuItem("Mark Center")
    if item then
        AddCommandForCoalition(coalition.side.BLUE, util, item, onMarkCenter)
    end

    item = CreateMenuItem("Random Message")
    if item then
        AddCommandForCoalition(coalition.side.BLUE, demo, item, onCycleMsg)
    end
end
