...
local function getFirstUnitId(grp)
    ...
    if not okU or not unit then return nil end
...
local function sectionMarksAndModify()
    ...
    CircleToAll(circleId, circleCenter, R * 0.6, COLORS.GREEN, rgba(0, 1, 0, 0.2), 1, true, "Radius demo")
    SetMarkupRadius(circleId, R * 0.3)

    -- Coalition/group marks
    local idCoal = NextId()
    MarkToCoalition(idCoal, "BLUE MARK", {x = CENTER.x - GRID, y = ALT, z = CENTER.z + R}, 2, true)

    local demoGroup = getGroupByName("HARNESS_TEST_GROUP")
    local groupId = getGroupId(demoGroup)
    if type(groupId) == "number" then
        local idGrp = NextId()
        MarkToGroup(idGrp, "GROUP MARK", {x = CENTER.x - GRID, y = ALT, z = CENTER.z + 2 * R}, groupId, false)
    end
...
local function sectionMarkupToAll()
    ...
    MarkupToAll(2, -1, NextId(), center, perimeter, {0, 1, 0, 1}, {0, 1, 0, 0.2}, 1, true, "Circle shape")
    ...
    -- Quad
    MarkupToAll(6, -1, NextId(), p1, p2, p3, p4, {0, 1, 1, 1}, {0, 1, 1, 0.2}, 0, true, "Quad shape")

    -- Freeform up to 6 points, then colors and lineType
    MarkupToAll(7, -1, NextId(), p1, p2, p3, p4, {x=CENTER.x + 1.5 * R, y=ALT, z=CENTER.z + 2.5 * R}, {x=CENTER.x + R, y=ALT, z=CENTER.z + 2.5 * R}, {1, 1, 0, 1}, {1, 1, 0, 0.2}, 4)
end
