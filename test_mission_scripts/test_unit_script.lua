...
local function tryGetFirstUnitOfGroup(groupName)
    ...
    if not okG or not grp then return nil end
    local okU, u = pcall(grp.getUnit, grp, 1)
    if okU then return u end
    return nil
end
...
local function vec3ToString(v)
    ...
    return string.format("{x=%.1f,y=%.1f,z=%.1f}", tonumber(v.x) or 0, tonumber(v.y) or 0, tonumber(v.z) or 0)
end
...
local function sectionLookupAndIdentity()
    ...
    OutText(string.format("Name=%s exists=%s id=%s num=%s callsign=%s objId=%s catEx=%s active=%s",
        tostring(unitName), tostring(exists), tostring(id), tostring(numberInGroup), tostring(callsign),
        tostring(objectId), tostring(categoryEx), tostring(active)
    ), 10)
...
local function sectionKinematics()
    ...
    if heading then OutText("Heading(deg)=" .. string.format("%.1f", heading), 6) end
    if vel then OutText("Vel=" .. vec3ToString(vel), 6) end
end
...
local function sectionAffiliation()
    ...
    OutText(string.format("Coalition=%s Country=%s Group?=%s", tostring(coal), tostring(country), tostring(group ~= nil)), 8)
end
...
local function sectionLifeFuel()
    ...
    OutText(string.format("Life=%s MaxLife=%s Fuel=%.2f InAir=%s",
        tostring(life), tostring(life0), tonumber(fuel) or -1, tostring(inAir)
    ), 10)
end
...
local function sectionSensorsRadar()
    ...
    if sensors then
        local count = 0
        for _ in pairs(sensors) do count = count + 1 end
    ...
    OutText(string.format("Radar active=%s target?=%s", tostring(active), tostring(target ~= nil)), 8)
end
...
local function sectionControllerAndEmissions()
    ...
    OutText(string.format("Controller acquired=%s cachedAgain=%s", tostring(ctrl1 ~= nil), tostring(ctrl2 ~= nil)), 6)
...
local function sectionAircraftAndCargo()
    ...
    OutText(string.format("Airbase?=%s CanShipLanding=%s HasCarrier=%s FuelLowState=%s",
        tostring(airbase ~= nil), tostring(canShip), tostring(hasCarrier), tostring(lowFuel)
    ), 8)
    ...
    OutText(string.format("NearestCargos=%d OnBoard=%d Capacity=%s DescentOnBoard?=%s",
        n1, n2, tostring(capacity), tostring(descentOnBoard ~= nil)
    ), 8)
end
...
local function sectionMisc()
    ...
    OutText(string.format("DrawArg0=%s Communicator?=%s Seats?=%s",
        tostring(draw0), tostring(comm ~= nil), tostring(seats ~= nil)
    ), 8)
end
