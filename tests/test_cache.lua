...
function TestCache:setUp()
    ...
    self:mock('timer.getTime', function() return self.currentTime end)
    
    -- Create mock objects
    self.mockUnit = {
        getName = function() return "TestUnit" end,
        isExist = function() return true end,
        getTypeName = function() return "F-16C" end
    }
...
function TestCache:testCacheDecorator()
    ...
    local cachedGetUnit = CacheDecorator(
        mockGetByName,
        function(name) return name end,  -- Cache key is just the name
        "unit"
    )
