...
function TestMisc:testFilterTable()
    ...
    -- Filter even values
    local evens = FilterTable(t, function(v, k) return v % 2 == 0 end)
    lu.assertEquals(TableSize(evens), 2)
    lu.assertEquals(evens.b, 2)
    lu.assertEquals(evens.d, 4)
    
    -- Filter by key
    local vowels = FilterTable(t, function(v, k) return k == "a" or k == "e" end)
    lu.assertEquals(TableSize(vowels), 2)
    lu.assertEquals(vowels.a, 1)
    lu.assertEquals(vowels.e, 5)
    
    -- Invalid input
    lu.assertEquals(FilterTable(nil, function() return true end), {})
